...
/**
 * This class, DateSelectWhatSellsController, handles when a user sets the date for the what sells together report. 
 * Once the date is set and they hit the submit button, they will be directed to the what sells together table view. 
 * 
 * @author Molly Frost - 731002331 315 - Project 2
 *
 */

public class DateSelectWhatSellsController implements EventHandler<ActionEvent>{
	...
	public static Timestamp startDateWhatSells;
	public static Timestamp endDateWhatSells;
	...
	/**
	 *  Redirects user to what sells together table view if and only if they have set both dates.
	 * 
	 * @param event, ActionEvent
	 */
	public void submitButton(ActionEvent event) {
		if(startDateFXML.getValue() == null) {
			...
			startDateWhatSells = Timestamp.valueOf(x.atStartOfDay());
			x = endDateFXML.getValue();
			endDateWhatSells = Timestamp.valueOf(x.atStartOfDay());
		}
		try {
			AnchorPane root = new AnchorPane();
			
			FXMLLoader loader = new FXMLLoader();
			loader.setLocation( Main.class.getResource("view/WhatSells.fxml") );
			root = (AnchorPane) loader.load();
